...
GameGui.prototype.showEndGame = function(str) {
  ...
  //Displaying playersFinalResults
  (str == "Player 1" ) ? player1WinCount++ : player2WinCount++;
  var playersFinalResults = "<div>Total Played Games: "+ (parseInt(player1WinCount)+ parseInt(player2WinCount)) + "</div>";
  
  if(player1WinCount >= player2WinCount){
	playersFinalResults += "<div>Player 1 - Won Games: "+ player1WinCount + "</div>";  
	playersFinalResults += "<div>Player 2 - Won Games: "+ player2WinCount + "</div>";  
  }else{	  
	playersFinalResults += "<div>Player 2 - Won Games: "+ player2WinCount + "</div>";  
	playersFinalResults += "<div>Player 1 - Won Games: "+ player1WinCount + "</div>";  
  }
 
  document.getElementById("playersFinalResults").innerHTML = playersFinalResults;
  setCookie("player1WinCount", player1WinCount, 24*7*60);
  setCookie("player2WinCount", player2WinCount, 24*7* 60);
}
